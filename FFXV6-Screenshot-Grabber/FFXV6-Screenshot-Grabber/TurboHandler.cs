...
namespace FFXV6_Screenshot_Grabber
{
    ...
    {
        public void configureTurbo(List<String> screenshotNames, string currPath, string newPath) { // called when you click the 'save all turbo' button
            foreach(var chunk in screenshotNames.Chunk(Environment.ProcessorCount)) { // splits the total amount of screenshots in to chunks to the amount of your CPU core count (not logical cores), and runs for each on this
                TurboWorker newWorker = new(chunk.ToList(), currPath, newPath); // create a new worker with it's specific chunk
    ...
    internal class TurboWorker // class which defines the worker itself
    {
        ...
        public TurboWorker(List<string> screenshotList, string currentPath, string newPath) // constructor
        {
